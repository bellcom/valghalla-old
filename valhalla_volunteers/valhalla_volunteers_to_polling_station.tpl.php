...
<?php foreach($posts_to_fill as $i => $post): ?>
  <dl class="clearfix" id="volunteer-station-list-item-<?php print $i ?>">
    ...
    <dd>
      ...
      <a <?php if(isset($existing[$i])): ?>style="display:none";<?php endif; ?> href="/ajax/volunteers/station/list/<?php print $party_id ?>/<?php print $station_id ?>/<?php print $i ?>/<?php print $post['title'] . '-' . $post['party_id'] ?>" class="add<?php print (isset($existing[$i]) ? '' : '') ?>">Vælg</a>
